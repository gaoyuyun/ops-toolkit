...
当前版本为 `0.1.3`。入口 `opsctl` 启动时会显示工具版本和检测到的平台；不需要联网的模块在发布包离线解压后可直接运行。
...
```bash
bash <(curl -fsSL https://github.com/gaoyuyun/ops-toolkit/releases/download/v0.1.3/bootstrap.sh) --help
```
...
```bash
curl -fSLO https://github.com/gaoyuyun/ops-toolkit/releases/download/v0.1.3/ops-toolkit-v0.1.3.tar.gz
curl -fSLO https://github.com/gaoyuyun/ops-toolkit/releases/download/v0.1.3/ops-toolkit-v0.1.3.tar.gz.sha256
sha256sum -c ops-toolkit-v0.1.3.tar.gz.sha256
tar -xzf ops-toolkit-v0.1.3.tar.gz
./ops-toolkit-v0.1.3/bin/opsctl --help
```
...
RELEASE_BASE_URL=https://github.com/gaoyuyun/ops-toolkit/releases/download/v0.1.3 \
