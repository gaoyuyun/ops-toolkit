...
RELEASE_BASE_URL=https://github.com/OWNER/ops-toolkit/releases/download/v0.1.1 \
  "$root/scripts/build-release.sh"
(
  cd "$root/dist"
  sha256sum -c ops-toolkit-v0.1.1.tar.gz.sha256
)
tar -xzf "$root/dist/ops-toolkit-v0.1.1.tar.gz" -C "$temp"
"$temp/ops-toolkit-v0.1.1/bin/opsctl" --version | grep '^opsctl 0\.1\.1' >/dev/null
if tar -tzf "$root/dist/ops-toolkit-v0.1.1.tar.gz" | grep -E '\.py$|requirements\.txt|proxy-test|MIGRATION\.md' >/dev/null; then
...
"$root/dist/install.sh" --help | grep 'Install or upgrade ops-toolkit 0.1.1' >/dev/null
...
"$temp/install-bin/opsctl" --version | grep '^opsctl 0\.1\.1' >/dev/null
[[ $(readlink "$temp/install/current") == "$temp/install/ops-toolkit-v0.1.1" ]]
